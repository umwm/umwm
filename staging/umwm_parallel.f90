...
contains
  ...
  pure function tile_indices_2d(dims) result(indices)
    !! Given an input x- and y- dimensions of the total computational domain
    !! [im, jm], returns an array of start and end indices in x- and y-,
    !! [is, ie, js, je].
    integer, intent(in) :: dims(2)
    integer :: indices(4)
    integer :: tiles(2), tiles_ij(2)
  ...
  pure function tile_neighbors_1d() result(neighbors)
    !! Returns the image indices corresponding to left and right neighbor tiles.
    integer :: neighbors(2)
    integer :: left, right
  ...
  pure function tile_ij2n(ij) result(n)
    !! Given tile indices in a 2-d layout, returns the corresponding tile index
    !! in a 1-d layout:
    !!
    !!    +---+---+---+
    !!  2 | 4 | 5 | 6 |
  ...
  pure function tile_neighbors_2d(periodic) result(neighbors)
    ...
    integer :: neighbors(4)
    integer :: tiles(2), tiles_ij(2), itile, jtile
    integer :: left, right, down, up
    integer :: ij_left(2), ij_right(2), ij_down(2), ij_up(2)
